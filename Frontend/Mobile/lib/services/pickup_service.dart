...
class PickupService {
  static String baseUrl = 'http://3.6.88.253:5000'; // For Android emulator
  ...
  /// Creates an instant pickup request.
  ///
  /// Throws an [Exception] if the authentication token is not found or if the
  /// request fails.
  ///
  /// [wasteTypes] is the list of waste types for the pickup request.
  ///
  /// [latitude] and [longitude] are the coordinates of the pickup location.
  ///
  /// [address] is the address of the pickup location.
  ///
  ...
  static Future<Map<String, dynamic>> getRequestedPickupDetails(
    ...
    try {
      final response = await http.get(
        Uri.parse(
            'http://3.6.88.253:4000/api/users/customer/requested/$userId'),
        headers: {
          'Authorization': 'Bearer $authToken',
          'Content-Type': 'application/json',
        },
  ...
  static Future<void> updateCollectorLocation(
    ...
    try {
      ...
      final response = await http.patch(
        Uri.parse('http://3.6.88.253:4000/api/users/collectors/location'),
        headers: {
          'Authorization': 'Bearer $authToken',
          'Content-Type': 'application/json',
        },
  ...
  static Future<LatLng?> getCollectorLocation(String collectorId) async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
            'http://3.6.88.253:4000/api/users/collectors/location?collectorId=$collectorId'),
        headers: {
          'Authorization': 'Bearer $authToken',
          'Content-Type': 'application/json',
        },
